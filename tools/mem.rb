...
def mem(nc, nnode, pm_factor= 3)
  ...
  part_alloc_factor= 1.2

  puts "Memory estimate for nc= #{nc}, #{nnode} nodes, pm_factor = #{pm_factor}" 
  ...
  mm2= (pm_factor*nc)**2*(pm_nx/2+1)*2*4        # PM mesh for density

  printf("%.1f MB for %d particle\n", mp/mbyte, np)
  printf("%.1f MB for communication buffer\n", mb/mbyte)
  printf("%.1f MB for one PM mesh (mem1)\n", mm/mbyte)

  # memory for LPT
  m_lpt= nc**2*nx*4*12
  printf("%.1f MB for 2LPT (mem1)\n", m_lpt/mbyte)
  
  # memory for snapshot
  m_snp= part_alloc_factor*nc*nc*(nx+1)*sizep_min

  printf("%.1f MB for snapshot (mem2)\n", m_snp/mbyte)

  # memory shared for PM 2LPT/Particle Mesh/FoF halo finder/snapshot
  mem1= [m_lpt, mm].max
  mem2= [m_snp, mm2].max

  printf("%.1f MB for mem1 (PM/LPT)\n", mem1/mbyte)
  printf("%.1f MB for mem2 (PM/snapshot)\n", mem2/mbyte)

  printf("%.3f GB per core in total (particle+buffer+mem1+mem2)\n", (mp + mb + mm + mm2)/gbyte)
