...
int read_parameter_file(const char filename[], Parameters *const param)
{
    ...
    while (fgets(buf, 1023, fp))
    {
        ...
        if (strcmp(name, "nc") == 0)
            ...
            param->lightcone_basename = get_string(p, &param->strlen_lightcone_basename);
        else if (strcmp(name, "use_solve_growth") == 0)
            param->use_solve_growth = get_int(p);
        else if (strcmp(name, "de_w_add10") == 0)
        {
            printf("check %s:   value = %s \n", name, p);
            param->de_w = get_double(p) - 10.;
            printf("after get_couble: param->de_w = %lg\n", param->de_w);
        }
        else if (strcmp(name, "omega_l") == 0)
            param->omega_l = get_double(p);
        else if (strcmp(name, "omega_k") == 0)
            param->omega_k = get_double(p);
        else
            msg_abort(1007,
