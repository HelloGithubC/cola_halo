...
static double read_double(lua_State* L, const char name[], bool required, double default_val)
{
  lua_getglobal(L, name);
  if(!lua_isnumber(L, -1)) {
    if (!required) {
...
static int read_bool(lua_State* L, const char name[], bool required)
{
  lua_getglobal(L, name);
...
int read_parameter_file(const char filename[], Parameters* const param)
{
  ...
  if(param->fof_filename) {
    param->fof_linking_factor= read_double(L, "linking_factor", true, 0.0);
  ...
  if(param->subsample_filename) {
    param->subsample_factor= read_double(L, "subsample_factor", true, 0.0);
  ...
  if(param->snapshot_filename || param->init_filename) {
    param->write_longid= read_bool(L, "write_longid", true);
