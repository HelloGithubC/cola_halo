...
const int gb_use_numerical_q1q2 = 1;
	       ...
	       const float avel1)
{
  ...
  Om= Omega_m;
  const float Om143 = pow(Om/(Om + (1 - Om)*A*A*A *pow(A,-3.*(1.+gb_w))  ), 1.0/143.0);
  ...
  //growth2 = -3/7*growth1**2 * om^{-143}
  // printf("q2, q2  (original) = %f %f \n", q1, q2);
 

  /////////////////////////////////////////////////////////////////
  if(gb_use_numerical_q1q2) {
      double z, Da, Ea, QdDda, QdEda;
      float q1_n, q2_n, factor = 1./A; 
      z = 1./A -1.; intpl_D_E_Qs(z, &Da, &Ea, &QdDda, &QdEda);
      gb_Tsquare_DE(A, Da, Ea, QdDda, QdEda, &q1_n, &q2_n);
      q1_n *= factor; q2_n *= factor;
      msg_printf(debug, "  (use numerical q1, q2!) check difference: \n\tq1, q1_numerical = %f %f (diff=%f%%) \n\tq2, q2_numerical = %f %f (diff=%f%%)\n", q1,q1_n, (q1_n-q1)/q1*100., q2, q2_n, (q2_n-q2)/q2*100.); // xiaodong add them
      q1=q1_n; q2=q2_n;
   //   exit(0);
  }
  ///////////////////////////////////////////////////////////////
...
float growthD(const float a) { // growth factor for LCDM
    ...
    if( gb_use_solve_growth){
    	double Da, Ea, QdDda, QdEda; intpl_D_E_Qs((double)(1./a-1.), &Da, &Ea, &QdDda, &QdEda);
 //   	printf("(grwothD) a, growthD=growthDtemp/growthDtmp(1.0) (original cola), Da (our rlt) = %f  %f %f | difference = %f%% \n", a, growthDtemp(a) / growthDtemp(1.0), Da, (1.-Da/growthDtemp(a)*growthDtemp(1.0))*100.);  // xiaodong add this
	return  Da; }
    else{
...
float Qfactor(const float a) { // Q\equiv a^3 H(a)/H0.
    ...
    return sqrt(Om/(a*a*a)+(1.0-Om)*pow(a,-3.*(1.+gb_w)))*a*a*a;
}
...
float growthD2temp(const float a){
    ...
    float omega=Om/(Om + (1.0 - Om)*a*a*a*pow(a,-3.*(1.+gb_w)));
...
float growthD2(const float a) {// Second order growth factor
  if(gb_use_solve_growth){
  	double Da, Ea, QdDda, QdEda; intpl_D_E_Qs((double)(1./a-1.), &Da, &Ea, &QdDda, &QdEda);
 // 	printf("(grwothD2) a, growthD2 (original cola), Ea (our rlt)= %f  %f %f | difference = %f%% \n", a, growthD2temp(a) / growthD2temp(1.0), Ea, (1.-Ea/growthD2temp(a)*growthD2temp(1.0))*100.);    // xiaodong add this
	return Ea; }
  else
...
float growthD2v(const float a){ // explanation is in main()
  ...
  float omega=Om/(Om + (1.0 - Om)*a*a*a);
  if(gb_use_solve_growth){
    double Da, Ea, QdDda, QdEda, f1; intpl_D_E_Qs((double)(1./a-1.), &Da, &Ea, &QdDda, &QdEda);
    f1 = Qfactor(a)*(d2/a)*2.0*pow(omega, 6.0/11.); 
    // we highly suspect that growthD2v is just QdEda!!!
 //   printf("(grwothD2v) a, growthD2v (original cola), QdEda (our rlt) = %f  %f %f | difference = %f%%  \n", a, f1, QdEda, (f1-QdEda)/QdEda*100); // xiaodong add this
    return QdEda; }
  else
...
float decayD(float a){ // D_{-}, the decaying mode
    ...
    return sqrt(Om/(a*a*a)+(1.0-Om)*pow(a,-3.*(1.+gb_w)));
}
...
{ // returns Q*d(D_{+}^nGrowth*D_{-}^nDecay)/da, where Q=Qfactor(a)
  ...
  if(gb_use_solve_growth){
    double Da, Ea, QdDda, QdEda, f1; intpl_D_E_Qs((double)(1./a-1.), &Da, &Ea, &QdDda, &QdEda);
    f1 = (pow(decayD(a),-1.0 + nDecay)*pow(growthD(a),-1.0 + nGrowth)*(nGrowth*Nn- (3.0*(nDecay + nGrowth)*Om*growthD(a))/(2.*a)));  
 //   printf("(DprimeQ) a, DprimeQ (original cola), QdDda (our rlt) = %f  %f %f | difference = %f%%  \n", a, f1, QdDda, (f1-QdDda)/QdDda*100); // xiaodong add this
    return QdDda;}
  else 
...
void cola_set_snapshot(const double aout, Particles const * const particles, Snapshot* const snapshot)
{
  ...
  const float Om143= pow(Om/(Om + (1 - Om)*A*A*A*pow(A,-3.*(1.+gb_w))  ), 1.0/143.0);  //, 1.0/143.0);
  const float dda= Sphi(AI, AF, A);
  const float growth1=growthD(A);

  msg_printf(normal, "                   (cola_set_snapshot) Growth factor of snapshot %f (a=%.3f)\n",
	             growthD(AF), AF);

  float q1=1.5*Om*growth1;
  float q2=1.5*Om*growth1*growth1*(1.0 + 7.0/3.0*Om143);
  //growth2 = -3/7*growth1**2 * om^{-143}
  msg_printf(verbose, "q2, q2  (original) = %f %f \n", q1, q2);
  if(gb_use_numerical_q1q2){
    double z, Da, Ea, QdDda, QdEda;
    float q1_n, q2_n, factor = 1./A; 
    z = 1./A -1.; intpl_D_E_Qs(z, &Da, &Ea, &QdDda, &QdEda);
    gb_Tsquare_DE(A, Da, Ea, QdDda, QdEda, &q1_n, &q2_n);
    q1_n *= factor; q2_n *= factor;
    msg_printf(verbose, "  (use numerical q1, q2!) check difference: \n\tq1, q1_numerical = %f %f (diff=%f%%) \n\tq2, q2_numerical = %f %f (diff=%f%%)\n", q1,q1_n, (q1_n-q1)/q1*100., q2, q2_n, (q2_n-q2)/q2*100.); // xiaodong add them
    q1=q1_n; q2=q2_n;
    msg_printf(verbose, "q2, q2  (original) = %f %f \n", q1, q2);
    //exit(0);
  }
  ...
  const float Dv=DprimeQ(aout, 1.0); // dD_{za}/dy
//  printf("Before growthD2v ...\n");
  const float Dv2=growthD2v(aout);   // dD_{2lpt}/dy
//  printf("After  growthD2v ...\n");


  const float AC= particles->a_v;
  const float dyyy=Sq(A, AF, AC);

  // we need to write a code computing T^2 D1, T^2 D2
